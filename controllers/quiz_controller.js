...
//Busco usuarios
  exports.owner = function(req,res){
  	var s='%'+req.query.owner+'%';
  	models.User.findAll({where: ["username like ?", 
  	s]}).then(function(quizusers){
     res.render('owner',{users:quizusers,errors: []});
  	});
  };
//Busco pregunta por usuario
exports.ownerq = function(req,res){
	var s='%'+req.query.ownerq+'%';
	var x;
	models.User.find({where: ["username like ?", 
  	s]}).then(function(quis){
  		if(quis){
  		models.Quiz.findAll({where: ["UserId like ?", 
  	quis.id]}).then(function(quices){
  		res.render('quizes/index',{quizes:quices,errors:[]});
  	});}
  	res.render('quizes/index',{quizes:[],error:[]});
  });
};
