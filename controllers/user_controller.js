	...
	user.validate().then(function(err){
		if (err) 
			...
			.then( function(){// crea la sesión para que el usuario acceda ya autenticado y redirige a /
				var timeaux= new Date();
                req.session.segundos=timeaux.getSeconds();
                req.session.minutos=timeaux.getMinutes();
...
exports.destroy = function(req, res) {
	req.user.destroy().then( function() {// borra la sesión y redirige a /
		delete req.session.segundos;
        delete req.session.minutos;
