...
//Comprobación de salida de sesión
app.use(function(req,res,next){
 if(req.session.user){
    Aux= new Date();
    segundos=Aux.getSeconds();
    minutos=Aux.getMinutes();
    if(((req.session.segundos+60)<(segundos+
       (minutos-req.session.minutos)*60))){
       delete req.session.user;
       res.render('end',{errors:[]});}
       var timeaux2= new Date();
       req.session.segundos=timeaux2.getSeconds();
       req.session.minutos=timeaux2.getMinutes();
       next();
    }
    else{next();}
});
